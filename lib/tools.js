 ...
 *
 * The definitions are hand-rolled registry-ready objects (the same shape the
 * host's tool DSL produces) rather than built with `defineTool`, so this
 * package ships zero runtime dependencies. That is deliberate: the host
 * mounts `@deepseek-ai/dsh-tools` by name as a service plugin and resolves
 * module names from the profile directory — a plugin that lists that package
 * in `dependencies` installs a second copy into the profile, shadowing the
 * host's own service resolution and splitting module-scoped symbol identity.
 */
...
export function buildTools(getDir) {
  ...
  const logTool = {
    name: 'portfolio_log',
    description:
      "Append one dated history line to a portfolio project file's History section (creating the section if absent), optionally refreshing its last_checked field. Always use this instead of generic file-edit tools for history entries — it inserts at the top, keeping the History section newest-first, which hand-edits tend to break. The file must already exist.",
    parameters: {
      type: 'object',
      properties: {
        project: {
    ...
    async execute(args) {
      ...
      const rawEntry = requireString(args, 'entry')
      if (!SLUG.test(project)) {
        throw new Error(`invalid project slug "${project}" — expected a single file name segment matching ${SLUG}`)
      }
      const date = args.date ?? today()
      if (typeof date !== 'string' || !ISO_DATE.test(date)) throw new Error(`invalid date "${date}" — expected YYYY-MM-DD`)
