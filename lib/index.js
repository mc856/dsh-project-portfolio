...
/**
 * No schemastery Config export — this package deliberately ships zero runtime
 * dependencies (see lib/tools.js header). Config is validated by hand.
 */
function validateConfig(config) {
  const raw = config ?? {}
  if (raw.enabled !== undefined && typeof raw.enabled !== 'boolean') {
    throw new Error('dsh-project-portfolio: config.enabled must be a boolean')
  }
  if (raw.portfolioDir !== undefined && typeof raw.portfolioDir !== 'string') {
    throw new Error('dsh-project-portfolio: config.portfolioDir must be a string')
  }
  return { enabled: raw.enabled !== false, portfolioDir: raw.portfolioDir }
}
